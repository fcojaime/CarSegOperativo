...
// -------------------------
    // Write your own code here.
     ...
     global $Redirect;
     $Redirect="historico_cargas_rs2.php?s_id_periodo=".$NewRecord1->ListBox1->GetValue()."&s_opt_slas=SO";
...
function ValidaInfoCargada($id_proveedor,$id_registro )
{
	...
	$num_fallas=0;	
	$ssql = " select s.ID_PROYECTO, s.num_renglon from SO_Fact_Sabana  s where s.ID_Proyecto not in (select distinct ID_Proyecto from [SO_Fact_Requerimientos]  where  ID_Proyecto is not null)  and id_registro=$id_registro and id_usuario=$id_proveedor order by 2 "; 
	//echo $ssql;
	$db->query($ssql);
	while ($db->next_record() ) {  		         
	    $ID_PROYECTO=$db->f(0);	
