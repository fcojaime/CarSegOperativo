  ...
  //*****empieza la validación del contenido del archivo
    ...
    //	$validaciones[$n]="";
      ...
      $max_columnas=0;
			...
			//$cadiniSQL="insert into $nom_tabla  values ($id_registro,$id_usuario, $id_proveedor, $Fecha_Corte_BD,'P',$id_periodo, '$tipo_carga', $num_carga, '$opt_slas' ";
			$cadiniSQL="insert into $nom_tabla  values ($id_registro, $Fecha_Corte_BD, $id_periodo, $id_usuario ,$num_carga ";
			$cadValores=""; 
			$errores="NO"; 
			...
			for ($row =$num_rows_descartar; $row <= $highestRow; $row++) {
                ...
                $cadenaLabel.= "<td><strong>$v</strong></td>" ;
			      ...
			      //echo"</tr>" ;
           		...
           		//echo "<br> num_columnas_layout  $num_columnas_layout, num cols excel ". count($rowData[0])." row $row , num_rows_descartar $num_rows_descartar ,  highestRow $highestRow";
                  ...
                  //if ($contador<2)
                    ...
                    //$NewRecord1->Errors->addError("valor orig $v , valida cero ".$validacionCelda[0].", valida dos ".$validacionCelda[2]);
			            ...
			            if ($validacionCelda[0]==1) {
			              if ($validacionCelda[1]==" null " || $validacionCelda[1]=="") $validacionCelda[1]="&nbsp;";
                    ...
                    $cadenaLabel.= "<td  $cad_ancho_col>".$validacionCelda[1]."  </td>" ;
			              ...
			              $cadenaLabel.= "<td><strong><font color='red'>$validacionCelda[1] error $validacionCelda[3]  </font</strong></td>" ;
			              $errores_datos="SI"; 
			              $errores="SI";	
                    $erroresGeneral="SI";	 
                    $num_row_error.="$contador, ";            		              
			            }
          ...
          $contador++; 
             ...
             $cadSQLcompleta= $cadiniSQL." , ".($contador-1)." ".$cadValores.")";
			   		  // if ($contador<=2) ojo quitar
			          // $NewRecord1->Errors->addError($cadSQLcompleta);
              
					   $ConnCarga->query($cadSQLcompleta);
             //var_dump($ConnCarga);
					   if($ConnCarga->Errors->Count()!=0)
            ...
            $erroresGeneral="SI";              
             ...
             $NewRecord1->Label2->SetValue( $contenido_label2_ant."<br> <strong>Los valores marcados en rojo indican que hubo errores en la informacion de $leyenda en los registros $num_row_error </strong> ");
			       $NewRecord1->Errors->addError($cadFormatoRedini ."Los valores marcados en rojo indican que hubo errores en la información de $leyenda  en los registros $num_row_error ".$cadFormatofin);
