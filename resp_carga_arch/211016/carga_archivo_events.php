			...
			if($num_intentos_hechos<$num_intentos_permitidos){
				for ($nlayout=0; $nlayout<$numero_de_layouts; $nlayout++){	
					...
					$fecha_registro="'".date("Y/m/d H:i")."'";
			 		$ssql = "Update $nom_tabla num_carga=( 
			 		select ISNULL ( max(num_carga),0)+1 from $nom_tabla where id_proveedor=$id_proveedor  and id_periodo=$id_periodo)
			 		 		where id_usuario=$id_usuario and   id_registro=$id_registro   "; 
